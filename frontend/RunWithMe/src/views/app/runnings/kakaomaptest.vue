<template>
  <div class="main-content">
    ...
    <div class="wrap">
      ...
      <ul class="tests">
        <li v-for="(test,index) in tests" class="item" :key="index">
          <span class="rank">{{ test.id }}</span>
          <router-link :to="{ name: 'show', params: { id: test.id } }">
            <img v-bind:src="test.poster" class="poster" />
          </router-link>
          <div class="detail">
            <strong class="tit">{{ test.name }}</strong>
            <span class="rate"
              >예매율 <span class="num">{{ test.rate }}</span></span
            >
            <router-link
              :to="{ name: 'show', params: { id: test.id } }"
              class="link"
              >자세히보기</router-link
            >
          </div>
...
export default {
  ...
  created() {
    // 컴포넌트가 생성될 때, /api/tests에 요청을 보냅니다.
  ...
  methods: {
    ...
    startLocationUpdates() {
      ...
      this.watchPositionId = navigator.geolocation.watchPosition(
        ...
        { timeout: 5000, maximumAge: 0, distanceFilter: 1 }
      );
    },
    stopLocationUpdates() {
