    ...
    initMap() {
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(function (pos) {
          ...
          // this.watching();
          var cur_lat = document.getElementById("lat").innerText;
          var cur_lon = document.getElementById("lon").innerText;
          console.log(cur_lat);
          console.log(cur_lon);

          var locPosition = new kakao.maps.LatLng(cur_lat, cur_lon), // 마커가 표시될 위치를 geolocation으로 얻어온 좌표로 생성합니다
            message = '<div style="padding:5px;">여기임 ㅎㅇ</div>'; // 인포윈도우에 표시될 내용입니다
          ...
          function displayCircleDot(position, distance) {
            ...
            if (distance > 0) {
              ...
              var distanceOverlay = new kakao.maps.CustomOverlay({
                content:
                  '<div class="dotOverlay" style="position: relative;bottom: 10px; border-radius: 6px;  border: 1px solid #ccc;  border-bottom: 2px solid #ddd; float: left;  font-size: 12px;  padding: 5px;  background: #fff;">거리 <span class="number">' +
                  distance +
                  "</span>m</div>",
                position: position,
                yAnchor: 1,
          ...
          function getTimeHTML(distance) {
            ...
            var content = '<ul class="dotOverlay distanceInfo" style=" position: relative;'
  +'bottom: 10px;'
  +'border-radius: 6px;'
